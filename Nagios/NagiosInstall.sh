...
if [ -d "${INST_DIR}/share/images/logos" ]; then
    ...
    if [ ! -f py-logger-logo-icon.png ]; then
        echo "getting ${INST_DIR}/py-logger-logo-icon.png"
        sudo wget https://raw.githubusercontent.com/PHuhn/RaspberryPI/master/Nagios/py-logger-logo-icon.png
        sudo chown nagios:nagios py-logger-logo-icon.png
    fi
    cd "${INST_DIR}" || exit
else
...
ls -l ${INST_DIR}/bin/nagios \
    ...
    ${INST_DIR}/share/images/logos/py-logger-logo-icon.png
